...
- **Adaptive Layout:** Control placement is resolved dynamically from current window dimensions. Portrait or square windows align the controls (shutter and grid toggle) at the bottom center; landscape windows align the capture strip at the center end. Layout safe drawing padding prevents encroachment under system bars, cutouts, or gesture navigation zones.
