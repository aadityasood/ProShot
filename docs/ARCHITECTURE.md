...
- **Rule-of-Thirds Grid:** The grid is default-off and is drawn as a passive Canvas overlay. Its toggle state uses `rememberSaveable`; the Canvas does not intercept pointer input and adds no accessibility semantics nodes.
- **Accessibility:** The grid toggle uses localized descriptions and state, switch semantics, and a 48 dp minimum target. The shutter provides loading, ready, capturing, and disabled descriptions and uses an 84 dp target. Decorative Canvas elements do not create duplicate accessibility targets.
- **Accessible Capture Outcome Feedback:** Capture feedback is statefully managed via `CaptureFeedbackReducer` (`CaptureFeedbackState`: `Hidden`, `Capturing`, `Terminal`). Upon accepted shutter input, any transient tap-focus message is replaced and the UI enters capturing state exactly once with localized visible text "Taking photo..." (`R.string.taking_photo`). The capturing message remains stable through Camera2 capture, conversion, processing, saving, and preview recovery without surfacing intermediate coordinator progress strings. When `CameraCaptureRuntime.capture` returns, the UI transitions to a terminal state displaying the exact `CaptureResult.message` verbatim for 5,000 ms before auto-dismissing using a monotonically increasing event token. Dismissal validation prevents stale tokens or auto-dismiss policies from clearing newer capturing or terminal states. `CaptureFeedbackPill` renders the active feedback as one explicit content-description node with `LiveRegionMode.Polite`, maintaining existing placement, styling, and padding without direct accessibility manager announcements or duplicate semantics nodes. Tap focus ("Focus set") remains a separate 3-second visual message over which capture feedback takes priority.
